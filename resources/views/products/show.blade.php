...
<div class="container">
    ...
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb small">
            ...
            @endif
            <li class="breadcrumb-item active text-dark fw-semibold text-truncate" aria-current="page" style="max-width: 250px;">
                {{ $product->name }}
            </li>
        </ol>
    </nav>
    ...
    <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-5">
        <div class="card-body p-4 p-md-5">
            <div class="row g-4 g-lg-5 align-items-start">
                ...
                <div class="col-lg-6">
                    @if(isset($product->category_name))
                        <span class="badge bg-primary-subtle text-primary border border-primary-subtle fw-semibold px-3 py-2 rounded-pill mb-3">
                            <i class="fa-solid fa-tag me-1"></i> {{ $product->category_name }}
                        </span>
                    @endif

                    <h2 class="fw-extrabold text-dark mb-2 lh-sm">{{ $product->name }}</h2>
                    ...
                    <div class="bg-light p-3 rounded-3 mb-4 border">
                        <div class="d-flex align-items-baseline gap-3 mb-1">
                            <span class="display-6 fw-bold text-primary">₹{{ number_format($effectivePrice, 2) }}</span>
                            @if($hasDiscount)
                                <span class="fs-5 text-muted text-decoration-line-through">₹{{ number_format($product->price, 2) }}</span>
                            @endif
                        </div>
                        @if($hasDiscount)
                    ...
                    <div class="mb-4">
                        @if($product->stock > 0)
                            <div class="alert alert-success border-0 py-2 px-3 small d-inline-flex align-items-center gap-2 mb-0">
                        ...
                        @else
                            <div class="alert alert-danger border-0 py-2 px-3 small d-inline-flex align-items-center gap-2 mb-0">
                                <i class="fa-solid fa-circle-xmark fs-6 text-danger"></i>
                    ...
                    <form action="{{ route('cart.add') }}" method="POST" class="mb-4">
                        ...
                        <!-- Quantity Selector -->
                        <div class="mb-4">
                            <label class="form-label fw-semibold text-dark mb-2">Select Quantity</label>
                            <div class="input-group" style="width: 150px;">
                                <button type="button" class="btn btn-outline-secondary px-3" onclick="decrementQty()">
                                ...
                                <input type="number" 
                                       ...
                                       max="{{ $product->stock }}" 
                                       readonly>
                                <button type="button" class="btn btn-outline-secondary px-3" onclick="incrementQty()">
                        ...
                        <div class="row g-3">
                            <div class="col-6">
                                <button type="submit" 
                                        class="btn btn-outline-primary btn-lg w-100 py-3 fw-bold shadow-sm" 
                    ...
                    <div class="border rounded-3 p-3 bg-white shadow-xs">
                        <div class="row g-2 text-secondary small">
                            <div class="col-6 d-flex align-items-center gap-2">
                                ...
                                <span>Free Shipping &ge; ₹1,000</span>
                            </div>
                            <div class="col-6 d-flex align-items-center gap-2">
                            ...
                            <div class="col-6 d-flex align-items-center gap-2 mt-2">
                                <i class="fa-solid fa-lock text-info fs-5"></i>
